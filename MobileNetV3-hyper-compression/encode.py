...
import time
import argparse
from models import build_mobilenet_v3

def get_args():
    parser = argparse.ArgumentParser(description='Compress the parameters of MobileNetV3 to a dictory')
    parser.add_argument('--model_name', type=str, default='MobileNetV3', help='The original model to be compressed')
    parser.add_argument('--rect_l', type=float, default=0.8, help='The length of sides of the inner square')
    parser.add_argument('--num_inner_list', type=list, default=[1600], help='The number of the sample nodes in the inner square')
    parser.add_argument('--class_max', type=int, default=4, help='The maximum number of categories of points outside the inner square')
    ...
    parser.add_argument('--stop_threshold', type=float, default=[False, 0],
                        help='You can set a threshold so that when the compression loss of the current layer is greater than the threshold, the original parameters of the current layer will be directly saved, which may better maintain the performance of the model, but will reduce the compression multiple')
...
if __name__ == '__main__':
    ...
    Decode_Param_Path = f'./compressed_result/{args.model_name}_l_01'

    model = build_mobilenet_v3()
    model.load_state_dict(torch.load('Origin_Params.pth'))
